...
void scheduler_init(void) {
    k_memset(&main_scheduler, 0, sizeof(main_scheduler));
    main_scheduler.next_pid = 1;
    scheduler_initialized = 1;

    if (!create_process(idle_process, 255)) {
        kernel_panic("Unable to create idle process");
    }
}
...
struct process* create_process(void (*entry_point)(void), u32 priority) {
    ...
    k_memset(new_process, 0, sizeof(*new_process));
