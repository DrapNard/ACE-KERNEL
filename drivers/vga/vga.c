...
void vga_scroll(void) {
    memcpy(vga_buffer, vga_buffer + VGA_WIDTH,
...
// Manual variadic argument handling for x86_64
void vga_printf(const char* fmt, ...) {
    // On x86_64, the first 6 arguments are passed in registers:
    // RDI, RSI, RDX, RCX, R8, R9
    // Additional arguments are passed on the stack.
    // We manually access these registers/stack using inline assembly.

    // Pointer to the first variadic argument (after `fmt`)
    const char** arg_ptr = (const char**)(&fmt + 1);
